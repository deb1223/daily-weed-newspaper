...
export default function PriceTable({ embedded }: { embedded?: boolean }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <PriceTableInner embedded={embedded} />
    </Suspense>
  );
}

// ─── MAIN COMPONENT ───────────────────────────────────────────
function PriceTableInner({ embedded }: { embedded?: boolean }) {
  ...
  // ─── COMPARE ──────────────────────────────────────────────
  const handleCompare = (productName: string) => setCompareProduct(productName);
  ...
  return (
    <div className="prices-page">
      {/* Mini Masthead — only on /prices standalone page */}
      {!embedded && (
        <div className="mini-masthead">
          ...
          {!authLoading && (
            <div style={{
              ...
              {!user && (
                <Link href="/login?next=/" style={{ color: "var(--accent)", textDecoration: "none" }}>
                  Log in for Pro →
                </Link>
              )}
              {user && !isProUser && (
      ...
      {/* Page title — only on standalone page */}
      {!embedded && (
                    ...
                    {valueSortMsg && !isProUser && (
                      ...
                      >
                        ...
                        {user ? (
                          ...
                          <a href="/login?next=/" style={{ color: "var(--accent)", textDecoration: "underline" }}>
                            Log in or upgrade →
                          </a>
                        )}
                      </div>
      ...
      {/* Footer — only on standalone page */}
      {!embedded && (
